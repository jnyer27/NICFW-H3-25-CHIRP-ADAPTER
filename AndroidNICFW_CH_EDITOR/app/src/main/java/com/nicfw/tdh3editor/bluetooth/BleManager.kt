package com.nicfw.tdh3editor.bluetooth
...
/**
 * BLE manager that discovers and connects to the TD-H3 nicFW radio and compatible
 * BLE UART bridges (HM-10-style, Nordic UART, etc.).
 *
 * Scan filters advertise packets that list a known UART service UUID. After connect,
 * the first matching service is used; TX/RX characteristics are resolved by capability
 * (notify/indicate vs write).
 *
 * Default ATT payload is **20 bytes** until MTU negotiation succeeds; many adapters
...
class BleManager(private val context: Context) {
    ...
    /**
     * Start a BLE scan filtered by [SUPPORTED_UART_SERVICE_UUIDS] so only devices that
    ...
    companion object {
        /**
         * Known BLE UART / serial bridge service UUIDs (dongles, Nordic UART, nicFW TD-H3, etc.).
         * Discovery picks the first match in list order.
         */
 ...
 * BluetoothSocket blocking behaviour expected by [com.nicfw.tdh3editor.radio.Protocol].
 *
