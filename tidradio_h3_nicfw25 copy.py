...
STEP_LIST = ["5 kHz", "6.25 kHz", "10 kHz", "12.5 kHz", "25 kHz", "50 kHz", "100 kHz"]  # 10 Hz values: 5000, 6250, 10000, 12500, 25000, 50000, 100000
STEP_VALUES = [5000, 6250, 10000, 12500, 25000, 50000, 100000]
...
def _channel_to_memory(memobj, number, mem):
    ...
    mem.power = POWERLEVEL_LIST[int(_mem.txPower)] if int(_mem.txPower) < len(POWERLEVEL_LIST) else POWERLEVEL_LIST[0]
    ...
    # Extra: groups, bandwidth
    mem.extra = RadioSettingGroup("extra", "Extra")
    g = int(_mem.groups)
    g0, g1, g2, g3 = (g >> 0) & 0xF, (g >> 4) & 0xF, (g >> 8) & 0xF, (g >> 12) & 0xF
    for slot, val in [("group1", g0), ("group2", g1), ("group3", g2), ("group4", g3)]:
        rs = RadioSetting(slot, "Grp " + slot[-1], RadioSettingValueList(GROUPS_LIST, GROUPS_LIST[val]))
        mem.extra.append(rs)
    bw = "Narrow" if _mem.bandwidth else "Wide"
...
def _memory_to_channel(memobj, number, mem):
    ...
    _mem.txPower = POWERLEVEL_LIST.index(mem.power) if mem.power in POWERLEVEL_LIST else 0
...
class TH3NicFw25(chirp_common.CloneModeRadio):
    ...
    def get_memory(self, number):
        ...
        if number <= 2:
            try:
                rxf = int(_mem.rxFreq)
                txf = int(_mem.txFreq)
                with open("/Users/jasonrosenzweig/Documents/Cursor Projects/NICFW H3 25 Adapter/.cursor/debug-18c1cf.log", "a") as f:
                    f.write(json.dumps({"sessionId": "18c1cf", "hypothesisId": "H1", "location": "get_memory:entry", "message": "channel raw", "data": {"number": number, "index": index, "rxf": rxf, "txf": txf, "raw_len": len(raw) if raw else 0, "raw_first4": list(raw[0:4]) if raw and len(raw) >= 4 else None}, "timestamp": int(time.time() * 1000)}) + "\n")
            except Exception:
                pass
        # #endregion
        ...
        if number <= 2:
            try:
                with open("/Users/jasonrosenzweig/Documents/Cursor Projects/NICFW H3 25 Adapter/.cursor/debug-18c1cf.log", "a") as f:
                    f.write(json.dumps({"sessionId": "18c1cf", "hypothesisId": "H2", "location": "get_memory:exit", "message": "channel result", "data": {"number": number, "mem_freq": mem.freq, "mem_empty": mem.empty, "mem_name": getattr(mem, "name", "")[:20]}, "timestamp": int(time.time() * 1000)}) + "\n")
            except Exception:
                pass
        # #endregion
        return mem
    ...
    def get_settings(self):
        ...
        for i in range(16):
            vfo_a.append(RadioSetting("vfoState_groupModeChannels0_%d" % i, "Group Channel %d" % i, RadioSettingValueInteger(0, 197, int(s.vfoState_groupModeChannels0[i]))))
        ...
        for i in range(16):
            vfo_b.append(RadioSetting("vfoState_groupModeChannels1_%d" % i, "Group Channel %d" % i, RadioSettingValueInteger(0, 197, int(s.vfoState_groupModeChannels1[i]))))
        ...
        # Band Plan (20 plans at 0x1A02; frequencies in 10 Hz)
        ...
        for i in range(20):
            ...
            plan.append(RadioSetting("bandPlan_%d_startFreq" % i, "Start (10 Hz)", RadioSettingValueInteger(0, 0xFFFFFFFF, int(bp.startFreq))))
            plan.append(RadioSetting("bandPlan_%d_endFreq" % i, "End (10 Hz)", RadioSettingValueInteger(0, 0xFFFFFFFF, int(bp.endFreq))))
            plan.append(RadioSetting("bandPlan_%d_maxPower" % i, "Max Power", RadioSettingValueInteger(0, 255, int(bp.maxPower))))
            plan.append(RadioSetting("bandPlan_%d_txAllowed" % i, "TX Allowed", RadioSettingValueBoolean(bool(bp.txAllowed))))
            plan.append(RadioSetting("bandPlan_%d_wrap" % i, "Wrap", RadioSettingValueBoolean(bool(bp.wrap))))
            plan.append(RadioSetting("bandPlan_%d_modulation" % i, "Modulation (0-7)", RadioSettingValueInteger(0, 7, int(bp.modulation))))
            plan.append(RadioSetting("bandPlan_%d_bandwidth" % i, "Bandwidth (0-7)", RadioSettingValueInteger(0, 7, int(bp.bandwidth))))
        ...
        def apply_el(element):
            ...
            if name == "squelch":
                ...
                try:
                    ...
                    if 0 <= i < 16:
                        s.vfoState_groupModeChannels0[i] = int(val) & 0xFF
                ...
                try:
                    i = int(name.split("_")[-1])
                    if 0 <= i < 16:
                        s.vfoState_groupModeChannels1[i] = int(val) & 0xFF
                except (ValueError, IndexError):
                    pass
            elif name == "keyLock":
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        if 0 <= idx < 20:
                            ...
                            if field == "startFreq":
                                bp.startFreq = int(val) & 0xFFFFFFFF
                            elif field == "endFreq":
                                bp.endFreq = int(val) & 0xFFFFFFFF
                            elif field == "maxPower":
                                bp.maxPower = int(val) & 0xFF
                            elif field == "txAllowed":
                                bp.txAllowed = 1 if val else 0
                            elif field == "wrap":
                                bp.wrap = 1 if val else 0
                            elif field == "modulation":
                                bp.modulation = int(val) & 0x07
                            elif field == "bandwidth":
                                bp.bandwidth = int(val) & 0x07
                    except (ValueError, IndexError):
                        pass
            else:
                cal = self._memobj.calibration
